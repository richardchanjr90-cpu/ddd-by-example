...
So far we've put behaviour back into the model ([Part 1](01-rich-domain-model.md)), modelled a lifecycle
as a state machine ([Part 2](02-state-machine-enumeration.md)), and let aggregates communicate safely
(here). In **Part 4**, we tackle the part of a real product that changes most — the **business rules
themselves** — and model loyalty rules as versioned strategies that evolve without a single data
migration. [Read Part 4 →](04-versioned-rules-strategy.md)
