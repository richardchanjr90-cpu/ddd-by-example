...
namespace LoyaltyProgram.Http.ProductGroup
{
    public class ProductGroupPatchFunction : DisposeContextFilter<ILoyaltyTenantDbContext>
    {
        ...
        [FunctionName("ProductGroupPatchFunction")]
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function, "patch", Route = "productgroups")]
            [RequestBodyType(typeof(ProductGroupPatchViewModel), "ProductGroupPatchViewModel")] ProductGroupPatchViewModel model,
            HttpRequest req,
            ...
            ILogger log)
        {
            log.LogInformation($"{nameof(ProductGroupPatchFunction)} was triggered.");

            return await HandlerWrapper.WrapAsync(log, token, async () =>
            {
                return new OkObjectResult(await service.Patch(model));
            });
        }
    }
}